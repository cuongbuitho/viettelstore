				...
				<table class="table">
					...
					<tbody>
						<c:forEach var="category" items="${categories}">
							<tr>
								<td>${category.categorId}</td>
								<td>${category.categoryName}</td>
								<td><a href="/admin/edit?categorId=${category.categorId}"><button
											type="submit" name="action" value="Update"
											class="btn btn-primary">Edit</button></a> <a
									href="/admin/delete?categorId=${category.categorId}"><button
											type="submit" name="action" class="btn btn-danger">Delete</button></a></td>
							</tr>
						</c:forEach>
					</tbody>
								...
								<c:forEach var="pageIndex" begin="0" end="${maxPage - 1}">
									<c:if test="${pageIndex == active }">
										<li class="page-item active"><a class="page-link"
											href="/admin/pagec?pageIndex=${pageIndex}">${pageIndex + 1}</a></li>
									</c:if>
									<c:if test="${pageIndex != active }">
